...
public class IrcInputProcessor extends PDInputProcessor {
...
//        irc.send("#uabart", "SPD started!");
    ...
    void pressAction(final GameAction action) {
        Gdx.app.postRunnable(new Runnable() {
            @Override
            public void run() {
                eventKey.dispatch( new Key<>(-1, action, true ));
                eventKey.dispatch( new Key<>(-1, action, false ));
            }
        });
    }

    void pressCustomAction(final int keycode, final GameAction action) {
        Gdx.app.postRunnable(new Runnable() {
            @Override
            public void run() {
                eventKey.dispatch( new Key<>(keycode, action, true ));
                eventKey.dispatch( new Key<>(keycode, action, false ));
            }
        });
    }

    void pressKeyButton(int code) {
